...
private:
	...
	static const BlockOffset blockOffset;
...
public:
	...
	int GetBlockX(const int blockNum) const { return locX + blockOffset[(int)type][form][blockNum].x; }
	int GetBlockY(const int blockNum) const { return locY + blockOffset[(int)type][form][blockNum].y; }
